...
using Vyrse.Analyzers.Syntax;

namespace Vyrse.Analyzers;
...
[Generator]
public class ComVtableGenerator : IIncrementalGenerator
{
	internal static class Attributes
	{
		public const string Interface = "Windows.Win32.System.Com.InterfaceAttribute";
		public const string Method = "Windows.Win32.System.Com.MethodAttribute";
	}
	...
	{
		// Find partial classes that have [Interface] on any partial declaration.
		var targets = ctx.SyntaxProvider.ForAttributeWithMetadataName(fullyQualifiedMetadataName: Attributes.Interface, // e.g. "Vyrse.Analyzers.InterfaceAttribute"
		predicate: static (node, _) =>
		{
			// Very cheap syntactic filter: partial classes only.
			return node is ClassDeclarationSyntax cds && cds.Modifiers.Any(m => m.IsKind(SyntaxKind.PartialKeyword));
		}, transform: static (ctx, _) => ctx.TargetSymbol as INamedTypeSymbol).Where(static s => s is not null)!.Select(static (s, _) => s!).Collect(); // batch them so we can Distinct and emit once per type

		ctx.RegisterSourceOutput(targets, static (spc, list) =>
		{
			// Deduplicate across partials/files
			foreach (var type in list.Distinct<INamedTypeSymbol>(SymbolEqualityComparer.Default))
			{
				try
				{
					// Build the file (EmitForClass also decides header + #nullable)
					var unit = EmitForClass(type);
					if (unit is null) continue;

					// Hint name: "Namespace.Type[GenericArgs].generated.cs"
					var hint = ToHintName(type);

					// Let Roslyn format (LF line endings are analyzer-safe)
				...
				{
					spc.AddSource($"{type.Name}.error.generated.cs", $"// {ex}");
				}
			}
		});
	}
	...
	private static CompilationUnitSyntax? EmitForClass(INamedTypeSymbol cls)
	{
		...
		foreach (var m in methods)
		{
			...
			if (ps.Any(p => p.Kind == PKind.InBlit || p.Kind == PKind.RefBlit))
				needsUnsafe = true;

			if (ps.Any(p => p.Kind == PKind.ReadOnlySpanBlit || p.Kind == PKind.SpanBlit))
		...
		if (ShouldEmitCtor(cls))
		{
			...
			var ctor = ConstructorDeclaration(Identifier(cls.Name)).AddModifiers(Token(isUnknown ? SyntaxKind.ProtectedKeyword : SyntaxKind.PublicKeyword)).WithParameterList(ParameterList(SeparatedList(new[] { Parameter(Identifier("raw")).WithType(ParseTypeName("nint")), Parameter(Identifier("addRef")).WithType(PredefinedType(Token(SyntaxKind.BoolKeyword))).WithDefault(EqualsValueClause(LiteralExpression(SyntaxKind.FalseLiteralExpression))) })));

			// Only derived classes call base(raw, addRef)
			if (!isUnknown)
			{
				ctor = ctor.WithInitializer(ConstructorInitializer(SyntaxKind.BaseConstructorInitializer, ArgumentList(SeparatedList(new[] { Argument(IdentifierName("raw")), Argument(IdentifierName("addRef")) }))));
			}
	...
	private static void EmitMethod(INamedTypeSymbol cls, IMethodSymbol m, out MemberDeclarationSyntax cacheField, out MemberDeclarationSyntax impl)
	{
		...
		if (retAbi == "void")
			inner.Add(ParseStatement($"(({delegateSig}){cacheName})({string.Join(", ", abiArgs)});"));
		else
			inner.Add(ParseStatement($"{retAbi} __ret = (({delegateSig}){cacheName})({string.Join(", ", abiArgs)});"));
	...
	private static bool HasAttribute(ISymbol s, string fullName) => s.GetAttributes().Any(a => a.AttributeClass?.ToDisplayString() == fullName);
	private static bool HasInterfaceAttribute(ISymbol s) => HasAttribute(s, Attributes.Interface);
	private static bool HasMethodAttribute(ISymbol s) => HasAttribute(s, Attributes.Method);
	private static bool IsUnmanaged(ITypeSymbol t) => t.IsUnmanagedType;
	private static bool IsString(ITypeSymbol t) => t.SpecialType == SpecialType.System_String;
	private static bool IsScalar(ITypeSymbol t) => t.SpecialType != SpecialType.None || t.TypeKind == TypeKind.Enum;

	private static bool IsReadOnlySpan(ITypeSymbol t, out ITypeSymbol elem)
	{
		if (t is INamedTypeSymbol nt && nt.Name == "ReadOnlySpan" && nt.ContainingNamespace.ToDisplayString() == "System")
		{
	...
	private static bool IsSpan(ITypeSymbol t, out ITypeSymbol elem)
	{
		if (t is INamedTypeSymbol nt && nt.Name == "Span" && nt.ContainingNamespace.ToDisplayString() == "System")
		{
			elem = nt.TypeArguments[0];
			return true;
		}
	...
	private static bool IsUnknownDerived(ITypeSymbol t)
	{
		for (INamedTypeSymbol? x = t as INamedTypeSymbol; x != null; x = x.BaseType)
		{
			if (x.Name == "Unknown") return true;
		}

		return false;
	}

	private static bool IsNullableRefType(ITypeSymbol t) => t.IsReferenceType && t.NullableAnnotation == NullableAnnotation.Annotated;
	...
	private static List<PSpec> Classify(IMethodSymbol m, out bool returnsHresult)
	{
		...
		foreach (var p in m.Parameters)
		{
			...
			// COM interfaces
			if (rk == RefKind.Out && IsUnknownDerived(t))
			{
				list.Add(new PSpec(PKind.OutInterface, t, p.Name));
				continue;
			}

			if (IsUnknownDerived(t))
			{
	...
	private static int GetMethodSlot(IMethodSymbol m)
	{
		var attr = m.GetAttributes().First(a => a.AttributeClass?.ToDisplayString() == Attributes.Method);
		var ctor = attr.ConstructorArguments.FirstOrDefault();
		if (ctor.Value is int i && i >= 0) return i;

		// Allow named argument Index=...
		foreach (var kv in attr.NamedArguments)
		{
			if (kv.Key == "Index" && kv.Value.Value is int j && j >= 0)
				return j;
		}

		throw new InvalidOperationException($"[Method] on {m.ContainingType.Name}.{m.Name} is missing a non-negative index.");
	}
	...
	{
		var attr = m.GetAttributes().FirstOrDefault(a => a.AttributeClass?.ToDisplayString() == Attributes.Method);
		idx = -1;
		if (attr is null) return false;

		// ctor arg
		if (attr.ConstructorArguments.Length > 0 && attr.ConstructorArguments[0].Value is int c && c >= 0)
		{
			idx = c;
			return true;
		}
	...
	private static bool IsComMethodCandidate(INamedTypeSymbol type, IMethodSymbol m)
	{
		...
		if (HasMethodAttribute(m)) return true;
		var isPartialDecl = m.IsPartialDefinition && m.PartialImplementationPart is null;
	...
	private static string ToHintName(INamedTypeSymbol type) => type.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat).Replace("global::", "").Replace('<', '[').Replace('>', ']') + ".generated.cs";
	...
	{
		return cls.GetMembers().OfType<IMethodSymbol>().Any(m => m.Name == "OnCreate" && !m.IsStatic && m.ReturnsVoid && m.Parameters.Length == 2 && m.Parameters[0].Type.SpecialType == SpecialType.System_IntPtr && m.Parameters[1].Type.SpecialType == SpecialType.System_Boolean);
	}
}