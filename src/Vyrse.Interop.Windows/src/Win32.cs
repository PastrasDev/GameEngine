...
namespace Windows
{
	namespace Win32
	{
		...
		public static partial class Winuser
		{
			...
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			public static void ForceHideCursor(int cycles = 32) { for (var i = 0; i < cycles && ShowCursor(false) >= 0; i++) ; }

			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			public static void ForceShowCursor(int cycles = 32) { for (var i = 0; i < cycles && ShowCursor(true) < 0; i++) ; }
			...
			public static void SetClientSize(HWND hwnd, int width, int height)
			{
				...
				var borderX = rect.Width - clientRect.Width;
				var borderY = rect.Height - clientRect.Height;
