...
public sealed class MainChannelsView(EngineChannels c) : IMainChannels
{
	...
	public IRenderCommandBus RenderCommands
	{
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		get => c.RenderCommands;
	}
}
...
public sealed class GameChannelsView(EngineChannels c) : IGameChannels
{
	...
	public ISnapshotWrite<SceneView> SceneWriter
	{
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		get => c.Scene.Writer;
	}
...
{
	public ISnapshotRead<SceneView> SceneReader
	{
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		get => c.Scene.Reader;
	}
...
internal sealed class SpscRing<T> : IPipe<T>
{
	...
	{
		if (capacityPow2 <= 0 || (capacityPow2 & (capacityPow2 - 1)) != 0) throw new ArgumentException("capacity must be a power of two and > 0", nameof(capacityPow2));
