	...
	namespace Abstract
	{
		...
		public abstract class Registry<TSelf, TItem> : Registry where TSelf : Registry<TSelf, TItem> where TItem : notnull
		{
            ...
            private sealed class Segment
            {
                ...
                public bool Add(Type key, object value)
                {
                    ...
                    lock (_writeLock)
                    {
                        ...
                        var next = cur.WithAdded(key, value, out bool added);
                ...
                public bool Replace(Type key, object value)
                {
                    ...
                    {
                        var cur = Volatile.Read(ref _map);
                        var next = cur.WithReplaced(key, value, out bool replaced);
                ...
                public bool Remove(Type key)
                {
                    ...
                    {
                        var cur = Volatile.Read(ref _map);
                        var next = cur.WithRemoved(key, out bool removed);
			...
			private sealed class Map
			{
				...
				{
					int cap = NextPow2(Math.Max(8, capacity));
					_keys = new Type?[cap];
