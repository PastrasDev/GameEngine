...
[MustDisposeResource]
[CollectionBuilder(typeof(UnmanagedArrayBuilder), nameof(UnmanagedArrayBuilder.Create))]
public sealed unsafe class UnmanagedArray<T> : IDisposable, IEnumerable<T> where T : unmanaged
{
	public static UnmanagedArray<T> Empty { get; } = new(0, owns:false);

	public T* Ptr { get; private set; }
	public int Length { get; private set; }
	private bool _owns;

	public UnmanagedArray(int length, bool zeroInit = false) : this(length, owns:true, zeroInit: zeroInit) { }

	private UnmanagedArray(int length, bool owns, bool zeroInit = false)
	{
		Length = length;
		_owns  = owns;
		if (length == 0)
	...
	[MustDisposeResource]
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static UnmanagedArray<T> Allocate(int length, bool zeroInit = false) => length <= 0 ? Empty : new UnmanagedArray<T>(length, zeroInit);

	[MustDisposeResource]
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static UnmanagedArray<T> From(ReadOnlySpan<T> src)
	{
		if (src.Length == 0) return Empty;
		var arr = new UnmanagedArray<T>(src.Length);
	...
	public UnmanagedArray<T> Clone()
	{
		...
		var copy = new UnmanagedArray<T>(Length);
...
public static class UnmanagedArrayBuilder
{
	[MustDisposeResource]
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static UnmanagedArray<T> Create<T>(ReadOnlySpan<T> items) where T : unmanaged => UnmanagedArray<T>.From(items);
}