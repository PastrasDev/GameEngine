...
public sealed class Registry
{
	...
	public bool IsParent => _parent is { };
	...
	{
		var key = typeof(T);
		for (var r = this; r is { }; r = r._parent)
		{
	...
	private sealed class ServiceMap
	{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private (Type?[] keys, object?[] vals, int count) Rebuild(int capacity, Type? skipKey, Type? replaceKey,
		                                                          object? replaceVal)
		{
			...
			for (int i = 0; i < _keys.Length; i++)
			{
				...
				if (replaceKey is { } && ReferenceEquals(k, replaceKey))
